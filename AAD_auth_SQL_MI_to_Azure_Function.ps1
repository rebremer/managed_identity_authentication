...
$newAppRole.DisplayName = "Allow MSI SPN of SQL to authenticate to Azure Function using its MSI"
$newAppRole.Description = "Allow MSI SPN of SQL to authenticate to Azure Function using its MSI"
...
# $servicePrincipal=Get-AzADServicePrincipal -ApplicationId $appReg.AppId 

# 7. Set MI of SQL as only authorized user to log in web app (azure function) 
$azure_sql_resource = get-azsqlserver -resourcegroupname  $rg_name -name $sql_name

New-AzureADServiceAppRoleAssignment -ObjectId $azure_sql_resource.Identity.PrincipalId -Id $newAppRole.Id -PrincipalId $azure_sql_resource.Identity.PrincipalId -ResourceId $servicePrincipal.Id
